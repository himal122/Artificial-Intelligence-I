...
def make_xor_reliability_plot(train_x, train_y):
    ...
    # range of hidden layers width 1-10
    hidden_layer_width = list(range(1, 11))

    # store resutls
    successes = np.zeros(10) # no. of succefull run
    epochs = np.zeros((10, 10)) # epochs for each repetition for each width

    for h_nodes in hidden_layer_width:
        for repetition in range(10):
            # create MLP with h_nodes hidden nodes and reandom state = repetiotion
            ...
            # fit the modal to the XOR data
            xorMLP.fit(train_x, train_y)

            # measure training accuracy
            accuracy = 100 * xorMLP.score(train_x, train_y)

            # if 100% accurate record success and epochs
            if accuracy == 100:
                successes[h_nodes - 1] += 1
                epochs[h_nodes -1][repetition] = xorMLP.n_iter_

    # calculate success rate
    success_rate = successes # successes/10 / commenting this as this success rate [0.  0.1 0.2 0.5 0.7 0.9 0.8 1.  0.9 1. ] because
                             # the expection one in the markings is this [ 0  1  2  5  7  9  8 10  9 10].

    # calculate efficiency for successful runs, default 1000 if no successes
    ...
    fig, axs = plt.subplots(1, 2, figsize=(12, 5)) # You will need to change the definition of fig and ax e.g. fig, ax = plt.subplots()"
    ...
    axs[0].set_ylim(0, 20)  # Success rate is between 0 and 1
    axs[0].grid(True)
...
class MLComparisonWorkflow:
    ...
    def preprocess(self):
        ...
        # split the data to 70:30% with startify
        self.train_x, self.test_x, self.train_y, self.test_y = train_test_split(
            self.data_x, self.data_y, test_size=0.30, stratify=self.data_y, random_state=12345
        )

        # use min max scaler to normalize the features (range: 0-1)
        scaler = MinMaxScaler()
